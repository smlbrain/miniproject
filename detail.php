...
<html lang="en">
    ...
    <body>
        ...
        <section class="home" id="home" style="margin-bottom:245px;">
        <?php 
                $stmt = $pdo->prepare("SELECT * FROM ticket WHERE pid = ?");
                $stmt->bindParam(1, $_GET["pid"]); 
                $stmt->execute(); 
                $row = $stmt->fetch(); 
                ?>
                <div class="back_1" style="margin-top:20px;padding-left:-20px;">
                    <a href="../1.php"><img src="../pic/back.jpg" style="width: 60px;height:40px;cursor:pointer;"></a>
                </div>
                <div style="display:flex;margin-top:50px;align-items: center;justify-content: center;height: 600px;">
                <div style="box-shadow: 0 5px 10px rgba(0, 0, 0, 20%);">
                <img src='pic/<?=$row["pid"]?>.jpg'<?=$row["pid"]?>.jpg' width='350' height='500'>
                </div>
                <div class="form-container_2" style="padding:0px;">
                
                <a>
                <h3><?=$row["pname"]?></h3>
                วันที่แสดง : <?=$row["datedate"]?><br>
                สถานที่แสดง : <?=$row["avenue"]?><br>
                วันจำหน่ายบัตร : <?=$row["sellday"]?><br>
                ราคา : <?=$row["price"]?> บาท<br><br>
                <button class="more" style="cursor:pointer;" >กดเพื่อดูสปอยเพลง</button><br>                
                ...
                <?php 
                    ...
                    echo "<div style='padding-right:65px;'>";  
                    echo "<p style='margin-bottom:10px;border-left: solid 5px crimson;'>&nbsp;&nbsp;".$row["songlist"]."<br>"; 
                    echo "<p style='margin-bottom:10px;border-left: solid 5px crimson;'>&nbsp;&nbsp;".$row["songlist2"]."<br>";
                    echo "<p style='margin-bottom:10px;border-left: solid 5px crimson;'>&nbsp;&nbsp;".$row["songlist3"]."<br>";
                    echo "<p style='margin-bottom:10px;border-left: solid 5px crimson;'>&nbsp;&nbsp;".$row["songlist4"]."<br>";
                    echo "</div>";
                    echo "<div>";  
                    echo "<p style='margin-bottom:10px;border-left: solid 5px crimson;'>&nbsp;&nbsp;".$row["songlist5"]."<br>"; 
                    echo "<p style='margin-bottom:10px;border-left: solid 5px crimson;'>&nbsp;&nbsp;".$row["songlist6"]."<br>";
                    echo "</div>";
                    echo "</div>";
                ?>
